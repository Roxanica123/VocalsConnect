...
Odată ce toate fișierele audio au fost descărcate, separate în voce și instrumental iar fisierele ce conțin vocea au fost curațate prin
...
destul de flexibil încat să permită rularea de experimente pentru segmente de diferite dimensiuni și diferite tipuri de reprezentări ale
informațiilor audio. Pașii implementați pot fi observați în figura \ref{fig:f7}

\begin{figure}[h]
    \caption{Procesul de extragere al informațiilor din fisierele audio}
...
în funcție de forma sub care dorim să le salvăm, o listă de instanțe ale clasei Feature Function, o instanță a clasei Segment Splitter și,
opțional, un procent în cazul în care se dorește procesare unui subset al setului de date, opțiune utilă în procesul de testare al
funcționalităților pe diferite mașini.

Clasa Segment Splitter se ocupă de împărțirea semnalului audio în mai multe segmente, de dimensiuni fixe. La crearea unei instanțe pot fi
specificate numărul de secunde pe care îl vor avea segmentele și o listă de offset-uri de la care să inceapă separarea. De exemplu, după cum
...
Clasa Feature Function modelează un tip de informație pe care vrem să o extragem din segmentele audio. O instanță primește o funcție care,
...
Informațiile extrase pentru un segment pot fi salvate individual ca JSON sau Tensors serializați binar sau pot fi adăugate ca JSON într-un fișier ce
...
\begin{figure}[h]
    \caption{Spectrograma mel}
    \includegraphics[width=0.7\textwidth]{melspec.png}
    \centering
    \label{fig:f12}
\end{figure}

O spectrograma este o metodă de vizualizare a evoluției spectrului de frecvențe și intensitatea lor în timp care se obține aplicând o
...
diferitelor frecvențe la diferite faze de undă după cum se poate observa în figura \ref{fig:f8}. \footnote{\href{https://www.youtube.com/watch?v=XQ45IgG6rJ4&ab_channel=ValerioVelardo-TheSoundofAIValerioVelardo-TheSoundofAI}{Demystifying the Fourier Transform}}

\begin{figure}[h]
    \caption{Trecerea de la domeniul de timp la domeniul de frecvență}
    \includegraphics[width=0.65\textwidth]{ft.png}
    \centering
    \label{fig:f8}
\end{figure}

În urma aplicării aplicării unei transformări Fourier de scurtă durată obținem o evoluție în timp a frecvențelor prezente în semnalul audio.
...
centrul filtrelor triunghiulare Mel \ref{fig:f9} \footnote{http://siggigue.github.io/pyfilterbank/melbank.html}. 

\begin{figure}[h]
    \caption{Filtre Mel}
    \includegraphics[width=0.6\textwidth]{melbancs.png}
    \centering
    \label{fig:f9}
\end{figure}
...
sunetele, și are rezultate mai bune în literatură în problema clasificării genurilor muzicale\cite{resut1}, sau clasificării
 sunetelor în general\cite{large} decât spectrogramele simple sau MFCC atunci cand rețeaua este una convoluțională.

\section{MFCC}

\begin{figure}[h]
    \caption{MFCC}
    \includegraphics[width=0.9\textwidth]{mfcc.png}
    \centering
    \label{fig:f11}
\end{figure}
...
fiecare fereastră a semnalului audio și calcularea derivatelor de ordinul întai si ordinul al doilea, numite coeficienți delta, 
...
\footnote{\href{https://www.youtube.com/watch?v=4_SH2nfbQZ8&ab_channel=ValerioVelardo-TheSoundofAIValerioVelardo-TheSoundofAI}{Mel-Frequency Cepstral Coefficients Explained}}, 
această similaritate rezultată reprezentând un coeficient MFCC.

\begin{figure}[h]
    \caption{Transformarea cosinusoidală discretă}
...
timbrului și speech recognition în general, coeficienții delta și delta delta oferă informații temporale în legatură cu ritmul vorbirii.
Datorită acestor aspecte, coeficienți MFCC sunt frecvenți utilizați în problemele de speech recognition, principalul avantaj fiind dimensiunea
