\chapter{Arhitectura rețelei}
\section{Experimente}
\label{section:experiments}
În această secțiune sunt descrise diferite tipuri de rețele utilizate și arhitecturile utilizate, comparând rezultatele obținute și motivând
alegerea arhitecturii finale.
\subsection{CNN}
Rețelele  neuronale convoluționale (CNNs) sunt rețele neuronale utilizate, de obicei, pentru procesarea imaginilor. 
Prima aplicație ce are la bază rețelele convoluționale moderne \cite{cnns-first} a demonstrat faptul că o astfel de rețea 
poate recunoaște cifrele scrise de mână din setul de date MNIST cu o rată de eroare de doar 0.7\% (Boosted LeNet-4), un rezultat
semnificativ mai bun decât alte rețele ce nu aveau la bază straturi convoluționale și, de asemenea, cu un număr mai mic de parametrii
ce pot fi antrenați.

Ideea de bază a acestui tip de rețea este emularea modului în care oamenii procesează imaginile, prin 
identificarea unor structuri simple care agregate, definesc structuri mai complexe iar în final obiecte. Acest proces se bazează pe 
două elemente fundamentale, convoluția si agregarea (pooling).

\subsubsection{Convoluția}
Un strat convoluțional are la bază unul sau mai multe nuclee (kernels). Fiecare nucleu este o matrice, bidimensională sau
tridimensională, în funcție de numărul de canale al imaginii. Se calculează un produs scalar dintre nucleu și o submatricea 
imaginii de aceeași dimensiune cu cea a nucleului, rezultatul produsului fiind salvat in celula corespunzătoarea celulei 
care se află în centrul submatricii selectate. Procesul este ilustrat în figura \ref{fig:f15}. 
Pentru a calcula produsul 
scalar și pentru elementele de pe marginea matricii, în cazul în care nu se dorește reducerea dimensiunii, matricea va fi bordată.

\begin{figure}[h]
    \includegraphics[width=\textwidth]{kernels.png}
    \centering
    \captionsetup{justification=centering,font=small,labelfont=small}
    \caption{Produsul scalar dintre submatrice și nuclee (imagine preluată preluat din \textit{How does a convolutional neural network work}\protect\footnotemark[1])}
    \label{fig:f15}
\end{figure}
\footnotetext[1]{\url{https://youtu.be/t3qWfUYJEYU}}

Nucleul se deplasează cu un anumit număr de poziții (strides) pe verticală și pe orizontală. Acest factor poate modifica, din nou,
dimensiunea rezultatelor. Fiecare nucleu din stratul convoluțional va genera ca rezultat o matrice bidimensională.
Prin antrenare, nucleele acționează ca filtre ce vor fi modificate pentru a identifica structurile de baza ale imaginii,
linii și curbe în primele
straturi convoluționale și structuri mai complexe la adâncimi mai mari.

După un strat convolutional, se aplică o operație de agregare (pooling).

\subsubsection{Pooling}

Pooling-ul este o operație ce are ca scop reducerea dimensiunii reprezentării. Similar cu procesul de convoluție,
se stabilește o dimensiune de pooling și un stride. Diferența este faptul că, operația nu depinde de parametri ce pot
fi antrenați, este doar un calcul asupra elementelor dintr-o submatrice cu aceeași dimensiune ca dimensiune de pooling
stabilită. Această operație poate fi selectarea elementului maxim din submatrice (Max Pooling), calcularea mediei,
calcularea mediei ponderate în funcție de distanța de la centrul matricii, etc.
\pagebreak

Procesul de Max Pooling pentru o
dimensiune de (2, 2) și un stride de 2 este ilustrat în figura \ref{fig:f16}

\begin{figure}[h]
\includegraphics[width=0.5\textwidth]{pooling.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Max Pooling pentru dimensiunea (2, 2) și un stride de 2 (imagine preluată preluat din \textit{How does a convolutional neural network work}\protect\footnotemark[1])}
\label{fig:f16}
\end{figure}
\footnotetext[1]{\url{https://youtu.be/t3qWfUYJEYU}}

\subsubsection{Batch Normalization}
Batch Normalization este un proces prin care input-urile straturilor rețelei sunt normalizate, rezultatul fiind o antrenare mai
stabilă și rapidă a rețelei.

Normalizarea se obține calculând media batch-ului,
$\mu_{B} = \frac{1}{m}\sum^{m}_{i=1}x_{i}$, și deviația standard, $\sigma^{2}_{B} = \frac{1}{m}\sum^{m}_{i=1}(x_{i} - \mu_{B})^{2}$,
unde $m$ este numărul de elemente din batch. Se calculează $\hat x_{i}=\frac{x_{i}-\mu_{B}}{\sqrt{\sigma^{2}_{B}+\epsilon}}$, iar pe baza
acestor valori se calculează noile valori normalizate, $y_{i}=\gamma\hat x_{i} + \beta$, unde $\gamma$ și $\beta$ sunt paramentrii ce
pot fi antrenați. \cite{batch-norm}

\subsubsection{Arhitectura CNN implementată}

Având în vedere că spectrogramele Mel și coeficienții MFCC pot fi considerate reprezentări similare cu o
imagine cu un singur canal (grayscale), ce mențin aspectele de structură prezente în imagini, rețelele convoluționale
sunt populare nu doar în procesarea imaginilor, ci și în procesarea audio.
Arhitectura rețelei convoluționale implementate are 3 straturi convoluționale cu 32 de nuclee de dimensiunea (3, 3).
Fiecare strat convoluțional este urmat de un strat de Max Pooling cu dimensiunile (3, 3), (3, 3), respectiv (2, 2) apoi de
un strat de Batch Normalization. Output-urile sunt aplatizate și introduse ca input într-un strat dens conectat cu 64 de neuroni, urmat de
stratul de output.

\subsubsection{Dropout}
Dropout-ul este un procedeu prin care, în timpul antrenării, un procent din
neuronii rețelei dintr-un anumit layer sunt ignorați, alături de conexiunile
cu ceilalți neuroni. Acești neuroni sunt
selectați random la fiecare iterație. Rezultatul este prevenirea fenomenului
de overfitting deoarece devine imposibil ca rețeaua să devină dependentă
de un subset al neuronilor, atunci când aceștia pot deveni inactivi. Astfel,
este încurajată generalizarea. \cite{overfitting}

Din acest motiv, între straturile dens conectate am utilizat un strat de Dropout de 0.3.
\subsubsection{ReLU}
Funcțiile de activare pentru straturile convoluționale și primul strat dens sunt ReLU, $f(x) = max(0, x)$ deoarece
previne problema de dispariție a gradientului (vanishing gradient) și are o complexitate computațională scăzută.
\subsubsection{Softmax}
Ultimul strat este unul dens conectat cu un număr de neuroni egal cu numărul distincte de genuri și activare Softmax.
Fiecare valoare a output-ului va reprezenta un procent, sugestiv pentru gradul în care
un anumit gen se regăsește întru-un segment.
\subsubsection{Adam}
Optimizatorul utilizat a fost Adam cu o rată de învățare de 0.005 deoarece s-a demonstrat că are rezultate mai bune în
practică decât stochastic gradient descent \cite{adam}.

\pagebreak

\subsection{LSTM}
\subsubsection{Rețele neuronale recurente}
Rețelele neuronale recurente sunt un tip de rețele neuronale în care conexiunile dintre
noduri formează un graf orientat în care informația circulă în sensul axei temporale, prin
existența unor stări și inputuri ascunse. \cite{rnn}
Structura generală a unei rețele recurente este ilustrată în figura \ref{fig:f17}.

\begin{figure}[h]
    \includegraphics[width=\textwidth]{rnn.png}
    \centering
    \captionsetup{justification=centering,font=small,labelfont=small}
    \caption{Arhitectura tradițională unei rețele neuronale recurente (imagine preluată din \protect\cite{rnn})}
    \label{fig:f17}
\end{figure}

$a^{<t>}=g_{1}(W_{aa}a^{<t-1>}+W_{ax}x^{<t>}+b_{a})$ și $y^{<t>} = g_{2}(W_{ya}a^{<t>} + b_{y})$, unde $W_{aa},\ W_{ax}$ și
$W_{ya}$ sunt ponderile pentru conexiuni (weights) iar $b_{a}$ și $b_{y}$ sunt valorile bias. Atât ponderile, cât și valorile
bias sunt parametrii ce vor fi antrenați.

Rețelele neuronale recurente sunt des utilizate în problemele de procesarea limbajului natural,
de speech recognition și chiar de generarea de segmente muzicale, probleme în care ordinea informațiilor
are o mare importanță, deoarece iau în calcul aspectul
temporal al datelor.

O problemă a acestor tip de rețele este dificultatea accesării informațiilor care
se regăsesc la un număr mai mare de pași în trecut deoarece gradientul poate crește sau
descrește exponențial odată cu numărul mare de straturi care crește odată cu lungimea
datelor de intrare.

\subsubsection{LSTM}
Rețelele Long Short Term Memory (LSTM) sunt un subtip de rețele neuronale recurente
care rezolvă problema învățării dependențelor de lungă durată, prin adăugarea
unei stări menite să rețină informații pentru perioade mai lungi de timp.

Structura rețelei este similară cu cea a unei rețele recurente tradiționale, diferența
fiind la nivelul celulei care, acum nu mai conține doar un strat, ci patru straturi care
interacționează direct între ele \ref{fig:f18}.

\begin{figure}[h]
\includegraphics[width=0.8\textwidth]{lstm.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Arhitectura LSTM (imagine preluată din \protect\cite{lstm})}
\label{fig:f18}
\end{figure}

Zona roșie a figurii reprezintă elementele ce țin de memoria de lungă durată
a rețelei. Se observă faptul că asupra acestei stări se execută doar două operații.
Prima operație este legată de uitarea(eliminarea) informațiilor, iar a doua operație
este asociată cu adăugarea de noi informații.

Prima operație este o înmulțire bazată pe elemente (produs Hadamard) dintre
starea de scurtă durată curentă si output-ul stratului sigmoid, $f_{t} = \sigma(W_{f}\cdot[h_{t-1}, x_{t}] + b_{f})$, unde $[h_{t-1}, x_{t}]$ reprezintă
concatenarea output-ului stratului anterior cu input-ul stării curente.
Datorită proprietăților funcției de activare sigmoidă, valorile rezultate aparțin intervalului $[0, 1]$, unde $1$
semnifică faptul că informația va fi păstrată, iar pentru $0$ va fi complet eliminată \cite{lstm}.

A doua operație asupra stării ce ține de memoria de lungă durată este ilustrată în figura \ref{fig:f19}.

\begin{figure}[h]
\includegraphics[width=0.7\textwidth]{lstm1.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Actualizarea stării celulei (imagine preluată din \protect\cite{lstm})}
\label{fig:f19}
\end{figure}

Similar cu prima operație efectuată output-ul stratului sigmoid, $i_{t}$, va
acționa ca un filtru asupra valorilor $\widetilde{C}_{t}$ ce vor fi adunate la
starea $C_{t}$ a celulei. Astfel, se adaugă stării doar informațiile noi ce sunt
considerate relevante de rețea.

Zona verde din figura \ref{fig:f18} reprezintă output-ul celulei. Acest output va fi
construit prin aplicarea unei funcții de activare (tangenta hiperbolica) asupra stării celulei
și aplicarea unui filtru asupra rezultatului prin înmulțirea cu output-ul stratului sigmoid.

\subsubsection{Arhitectura LSTM implementată}
Având în vedere contextul temporal al datelor extrase din segmentele audio,
a fost construit un model bazat pe LSTM pentru a vedea dacă o procesare ce ține
cont de contextul temporal reușește să aibă performanțe mai bune decât o
rețea convoluțională, ținând cont de datele problemei abordate.

Modelul construit este format din doua straturi LSTM cu 64 de unități, urmate de un strat
dens conectat cu 64 de neuroni, un strat Dropout de 0.3 și un strat dens conectat de
output cu activare Softmax și un număr de neuroni egal cu numărul de genuri distincte
din setul de date.

\subsection{CNN-LSTM}

Combinarea rețelelor convoluționale cu rețele recurente este des utilizată în practică pentru
recunoașterea activităților din videoclipuri sau din secvențe de imagini, dar și pentru recunoașterea
sunetelor sau a genurilor muzicale.

Rețelele CNN-LSTM combină elementele de procesare a datelor bidimensionale sau
tridimensionale ale rețelelor convoluționale și aspectele temporale surprinse de
rețelele LSTM. Straturile convoluționale reduc dimensiunea inputurilor și extrag, în
același timp caracteristicile esențiale ale datelor de intrare. Caracteristicile
extrase de rețeaua CNN sunt apoi introduse într-o rețea LSTM care procesează evoluția
caracteristicilor, generând o predicție.

\subsubsection{Arhitectura CNN-LSTM implementată}
Rețeaua CNN are 4 straturi convoluționale cu 64 de nuclee de dimensiunea (3, 3). Fiecare strat
convoluțional este urmat de un strat de Max Pooling cu dimensiunile (2, 2), (3, 3), (4, 4), (4, 4),
și un strat de Batch Normalization și Dropout de 0.1. Output-ul rețelei CNN este aplatizat și introdus ca input în
rețeaua LSTM.

Rețeaua LSTM are aceeași arhitectură cu cea implementată în capitolul anterior. În continuare
sunt prezentate comparațiile între rețelele implementate.
\subsection{Comparații}
Rețelele CNN și LSTM au fost antrenate pe 20 de coeficienți MFCC pentru segmente de 5 secunde.
Dimensiunea datelor de intrare este de 128x20, iar dimensiunea datelor de ieșire corespunde numărului de genuri distincte prezise de rețea, 24 în cazul
comparațiilor.
Rezultatele antrenării pentru datele de antrenare și validare pot si observate în figura \ref{fig:f27} și \ref{fig:f26}. Pe datele de test
acuratețea prezicerii pentru CNN și LSTM a fost de $0.3466$, respectiv $0.4102$.

\begin{figure}[h]
\includegraphics[width=\textwidth]{cnn_5s.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Rezultatele antrenării pentru arhitectura CNN}
\label{fig:f27}
\end{figure}

\begin{figure}[h]
\includegraphics[width=\textwidth]{lstm_5s.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Rezultatele antrenării pentru arhitectura LSTM}
\label{fig:f26}
\end{figure}

Având în vedere doar aceste rețele, spectrogramele mel au o dimensiune prea mare pentru a putea fi clasificate, iar segmentele cu lungimi de ordinul
sutelor încep să pună probleme
rețelei LSTM.
În continuare, experimentele pentru rețeaua CNN-LSTM au ca date de input spectrograme Mel extrase din segmente de 15 secunde. Având în vedere faptul că
rețeaua CNN reduce dimensiunea datelor de input pentru rețeaua LSTM, lungimea datelor nu mai este o problema.
\pagebreak

Rezultatele antrenării pentru
datele de antrenare si validare pot fi observate
în figura \ref{fig:f28}, iar acuratețea pe datele de test a fost de $0.5124$

\begin{figure}[h]
\includegraphics[width=0.9\textwidth]{cnnlstm_15s_without dropout.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Rezultatele antrenării pentru arhitectura inițială CNN-LSTM}
\label{fig:f28}
\end{figure}

Având în vedere diferența mare dintre acuratețea pe datele de test și acuratețea pe datele de antrenare este evidentă prezența fenomenului de $overfitting$.
Pentru a preveni $overfitting\-ul$ am adăugat un strat de Dropout de $0.1$ după fiecare strat convoluțional, iar rezultatele s-au îmbunătățit considerabil după cum se poate
observa în figurile \ref{fig:f29} și \ref{fig:f30}, acuratețea
pe datele de test devenind $0.6140$.

\begin{figure}[h]
\includegraphics[width=0.9\textwidth]{cnnlstm_15s.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Rezultatele antrenării pentru arhitectura CNN-LSTM cu Dropout}
\label{fig:f29}
\end{figure}

\begin{figure}[h]
\includegraphics[width=0.9\textwidth]{cnnlstmcomparatie.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Comparația între acuratețea pe datele de validare între CNN-LSTM cu și fără Dropout după straturile convoluționale}
\label{fig:f30}
\end{figure}

\section{Selecția genurilor}
\label{section:genresselection}
Spotify oferă $126$ de genuri muzicale ca $seed$ pentru funcționalitatea de recomandări \footnote{https://developer.spotify.com/console/get-available-genre-seeds/}.
Dintre aceste genuri, au fost eliminate genurile care conțin în mare parte doar instrumental precum \textit{house}, \textit{techno} sau \textit{classical} și genurile muzicale
care ar introduce ambiguitate precum \textit{sad}, \textit{holiday}, \textit{rainy-day} sau \textit{workout}. După această filtrare inițială s-a ajuns la un set de date cu 46 de genuri muzicale.
Rețeaua CNN-LSTM cu Dropout descrisă în subcapitolul anterior a fost antrenată pe acest set de date, iar pentru o selecție ulterioară a genurilor cu scopul de a obține
o precizie mai bună a fost construită matricea de confuzie pe setul de date de test \ref{fig:f31}.

\begin{figure}[h]
\includegraphics[width=\textwidth]{cnn_lstm_faradropoutlaconv_64lstm_mel_specs_15s_46genuri_mai_multe_date.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Matricea de confuzie pentru 46 de genuri muzicale}
\label{fig:f31}
\end{figure}

Pe baza acestei matrici s-au eliminat progresiv genurile care erau prezise predominant greșit precum, cu valori mici pe diagonala matricii,
\textit{disco}, \textit{synth-pop} sau \textit{j-rock} și
genurile care generau \textit{false positives}, mai multe genuri fiind prezise fals aparținând unui anume gen. Acest lucru se poate observa pe liniile matricii
de confuzie. Printre aceste genuri s-au numărat \textit{folk}, \textit{hardrock} sau \textit{emo}. Selecția finală poate fi observată în figura \ref{fig:f32},
genul \textit{kids} fiind eliminat ulterior din cauza problemei descrise în capitolul \ref{section:kids}.

\begin{figure}[h]
\includegraphics[width=\textwidth]{cnn_lstm_cudropoutlaconv_64lstm_mel_specs_15s_25genuri_mai_multe_date.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Matricea de confuzie pentru 25 de genuri muzicale}
\label{fig:f32}
\end{figure}

\pagebreak
\section{Distribuția datelor}
\label{section:genuri}
După cum a fost menționat în capitolul \ref{section:building}, după curățarea datelor și eliminarea melodiilor care nu au destulă informație vocală,
nu mai există o distribuție egală a genurilor în ceea ce privește numărul de fișiere audio per gen. În plus, numărul de instanțe de antrenare pentru
fiecare gen nu este definit de numărul de fișiere audio, ci de numărul de segmente de 15 secunde în care este împărțit fiecare fișier. Astfel, distribuția
reală a datelor este reprezentată în figura \ref{fig:f33}

\begin{figure}[h]
\includegraphics[width=0.8\textwidth]{genres_segments.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Distribuția datelor pe genuri muzicale}
\label{fig:f33}
\end{figure}

Pentru a rezolva problema distribuției inegale am adăugat în pipeline-ul de încărcare a datelor în memorie opțiunea unui threshold. Astfel, genurile care
au mai puține instanțe decât valoarea threshold, nu vor fi luate în considerare, iar pentru genurile care au mai multe instanțe decât valoarea
threshold se va alege un subset de date de dimensiunea threshold-ului specificat. Utilizând această opțiune, rețeaua CNN-LSTM a fost antrenată pe un set de
date cu 498 de instanțe din fiecare gen (numărul de instanțe pentru genul blues). Rezultatele pe datele de antrenare și validate
pot fi observate în figura \ref{fig:f34}. Pe datele de test, acuratețea prezicerii a fost de $0.5616$. După cum se poate observa, rezultatele obținute
anterior, atunci când este folosit întreg setul de date, sunt mai bune decât atunci când numărul de instanțe este egal pentru toate genurile.
O acuratețe mai bună a prezicerii pentru cazul în care datele nu sunt distribuite egal poate fi observată și în matricele de confuzie din figura \ref{fig:f35}. Un posibil motiv este
numărul semnificativ mai mare de date atunci când este folosit întreg setul de date, astfel este evidențiată nevoia unui set mai mare de date.

\begin{figure}[h]
\includegraphics[width=\textwidth]{cnnlstm_equal_distribution.png}
\centering
\captionsetup{justification=centering,font=small,labelfont=small}
\caption{Rezultatele antrenării pentru arhitectura CNN-LSTM cu distribuția egală a datelor}
\label{fig:f34}
\end{figure}

\begin{figure}[h]%
\captionsetup{justification=centering,font=small,labelfont=small}
\centering
\subfloat[\centering Distribuție egală]{{\includegraphics[width=0.466\textwidth]{CNNLSTM-equal-distributions.png} }}%
\qquad
\subfloat[\centering Setul întreg de date]{{\includegraphics[width=0.467\textwidth]{cnn_lstm_cudropoutlaconv_64lstm_mel_specs_15s_25genuri_mai_multe_date.png} }}%
\caption{Comparație între matricele de confuzie}%
\label{fig:f35}%
\end{figure}

Astfel, modelul final utilizat în sistemul de recomandare a fost antrenat pe întreg setul de date, fără a ține cont de distribuția genurilor.
